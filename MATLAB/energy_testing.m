% we still don't understand why energy doesn't balance.
% so here we try to get rid of the linear time in the sim.
clear all;
tstart = 0;
tfinal = 20;
%%
...
while tstart < tfinal
    ...
    [t,states,te,ye,ie] = ode45(@hopperDynamics,[tstart,tfinal],y0,options,p);
    % set next round.
    tstart_old = t(1);
    y0 = states(end,:);
    tstart = t(end);
    ...
    if ~isempty(sol.ye) && p.fsm_state == p.FSM_FLIGHT
        strOut = hopperDynamicsFwd(sol.te,sol.ye(:),p);
        at_des = [at_des;sol.te,strOut.a_des];
    end;
    ...
    switch p.fsm_state %the state that we are just leaving!
        case p.FSM_THRUST
            p.T_s = (tstart-tstart_old) + p.T_compression;
            %             fprintf(['resetting T_s to ',num2str(p.T_s),'.\n']);
        case p.FSM_COMPRESSION
            p.T_compression = tstart-tstart_old;
    end;
